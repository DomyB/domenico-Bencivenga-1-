/* The Network page: every subject and post on the blog drawn as a living
   ...
   Colors come from the CSS variables named after each topic (--economics…). */
(function () {
  "use strict";

  var stage = document.querySelector(".network__stage");
  ...
  var editor = document.querySelector(".network-editor");
  var reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  var editing = /(^|&)edit(=|&|$)/.test(location.search.slice(1));
  var STORAGE_KEY = "network-drafts";
  ...
  function addNode(kind, key, label, raw, list) {
    var node = {
      ...
      list: list, topicKey: "", x: 0, y: 0, vx: 0, vy: 0, ax: 0, ay: 0, fx: null, fy: null,
  ...
  function collectEdges(withDrafts) {
    ...
    nodes.forEach(function (n) {
      var main = n.kind === "post" ? lookup[n.topicKey] : null;
      if (main && main.kind === "topic") add(n, main, "main");
    });
    nodes.forEach(function (n) {
      (withDrafts ? currentList(n) : n.list).forEach(function (name) {
  ...
  function refreshEdges() {
    var original = collectEdges(false);
    var current = editing ? collectEdges(true) : original;
    var had = {}, has = {};
    original.edges.forEach(function (e) { had[e.id] = true; });
    current.edges.forEach(function (e) { has[e.id] = true; e.draft = editing && !had[e.id]; });
    edges = current.edges;
    removed = editing ? original.edges.filter(function (e) { return !has[e.id]; }) : [];
  ...
  function endPress(event, cancelled) {
    ...
    if (clicked) {
      if (press.type === "mouse" && !editing) open(clicked);
      else if (selected === clicked && !editing) open(clicked);
      else select(clicked);
